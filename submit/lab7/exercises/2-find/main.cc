...
go(std::string containerSpec, std::vector<int> ints, int arg)
{
  if (containerSpec == "[]") {
	...
	for (int i = 0; i<(int) ints.size(); i++) {
		array[i] = ints[i];
		//from ints - > to array
		//go like ints[i] to the end
	}
	//pointer to first element loopFn()
	//pointer to last-1 element noLoopFn()
	int* p1 = &array[0];
	int* p2 = &array[ints.size()];
	loopFn(p1,p2,arg);
	noLoopFn(p1,p2,arg);
  }
...
{
  /*
	./main CONTAINER_SPEC INTS_DATA_FILE [INT]
	   0        1                2         3
	argv[0] = ./main
	argv[1] = CONTAINER_SPEC
	argv[2] = INTS_DATA_FILE
	argv[3] = [INT] - optional
  */
