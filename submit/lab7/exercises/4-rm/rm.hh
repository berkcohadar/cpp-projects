...
loopFn(Iterator begin, Iterator end, int arg)
{
  ...
  {
	if(arg != *p) vec.push_back(*p);
	else {
		if(isSeen) vec.push_back(*p);
		isSeen = 1;
	}
  }
...
{
  //@TODO
  Iterator temp = std::find(begin,end,arg);
  std::vector<int> vec(begin,temp);
  if(temp != end) std::copy(++temp, end, std::back_inserter(vec));
